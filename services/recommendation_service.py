...
class RecommendationService:
    ...
    PATTERN_CLASH_RULES = {
        ...
        "busy": ["stripes", "plaid", "polka", "floral", "geometric", "tropical", "animal"],
    }

    # Pattern compatibility scores
    PATTERN_COMPLEMENT_RULES = {
        "solid": ["stripes", "plaid", "polka", "floral", "animal", "camo", "tropical", "geometric"],  # Solid pairs with anything
        "minimal": ["stripes", "plaid", "polka", "floral", "animal", "geometric"],   # Minimal patterns (like thin stripes) pair well with most
        "striped": ["solid", "minimal"],  # Striped items pair well with solid or minimal patterns
        "textured": ["solid", "minimal"],  # Textured items (like knits) pair well with solid
    }
    
    @classmethod
    def are_patterns_compatible(cls, pattern1: Optional[str], pattern2: Optional[str]) -> Tuple[bool, float]:
        """
        ...
        if pattern1 in cls.PATTERN_CLASH_RULES and pattern2 in cls.PATTERN_CLASH_RULES[pattern1]:
            return False, 0.0
            
        if pattern2 in cls.PATTERN_CLASH_RULES and pattern1 in cls.PATTERN_CLASH_RULES[pattern2]:
            return False, 0.0
    ...
    def calculate_item_match_score(
        ...
        if item.pattern:
            ...
            if liked_pattern_key in user_style_profile and user_style_profile[liked_pattern_key] > 0:
                score_components["pattern_match"] = user_style_profile[liked_pattern_key]
                match_reasons.append(f"Features your preferred {item.pattern} pattern")
            
            # If this pattern matches their style profile
            elif pattern_key in user_style_profile and user_style_profile[pattern_key] > 0:
                score_components["pattern_match"] = user_style_profile[pattern_key]
                match_reasons.append(f"Has a {item.pattern} pattern that suits your style")
                
            # If this is in their preferred patterns list
            elif preferred_pattern_key in user_style_profile and user_style_profile[preferred_pattern_key] > 0:
                score_components["pattern_match"] = user_style_profile[preferred_pattern_key]
                match_reasons.append(f"Features your preferred {item.pattern} pattern")
            ...
            # Default pattern score if no specific preference
                ...
                if item.pattern.lower() == "solid":
                    ...
                    match_reasons.append("Versatile solid pattern that pairs with anything")
                else:
                    score_components["pattern_match"] = 0.5  # Neutral score for patterns
        else:
        ...
        if social_proof_context:
            ...
            if social_proof_score > 0:
                ...
                celebrity_reason = f"Inspired by {social_proof_context.celebrity}'s style"
        ...
        if social_proof_context and any("Inspired by" in r for r in unique_reasons):
            ...
            social_reasons = [r for r in unique_reasons if social_proof_context.celebrity in r]
            other_reasons = [r for r in unique_reasons if social_proof_context.celebrity not in r]
    ...
    def find_complementary_items(
        ...
        for candidate in complementary_candidates:
            ...
            pattern_compatible, pattern_score = RecommendationService.are_patterns_compatible(
        ...
        for category in needed_categories:
            ...
            for item in category_items:
                ...
                for outfit_item_id in outfit_items:
                    ...
                    if outfit_item:
                        # Check color compatibility
                        if not cls.are_colors_compatible(outfit_item.colors, item.colors):
                ...
                if not compatible_colors or (not compatible_patterns and pattern_total_score < 0.3):
                    continue
        ...
        if len(outfit_items) >= 3:
            ...
            if social_proof_context:
                ...
                social_matched_items = [
                    item for item in outfit_items_objects 
                    if hasattr(item, 'social_proof_match') and item.social_proof_match.get('match_score', 0) > 0.5
                ]
                ...
                if social_matched_items:
                    ...
                    if len(social_matched_items) > len(outfit_items_objects) / 2:
                        ...
                        match_reasons.insert(0, f"Partially inspired by {celebrity}'s look")
    ...
    def calculate_social_proof_match(
        ...
        if social_proof.outfit_tags:
            ...
            for tag in social_proof.outfit_tags:
                ...
                garment_keywords = [
                    ...
                    "top", "jeans", "shorts", "jumpsuit", "cardigan", "t-shirt", "tee",
                    "tank", "camisole", "hoodie", "turtleneck", "tunic", "leggings",
                    "culottes", "chinos", "joggers", "sweatpants", "maxi", "mini", "midi",
                    "slip", "bodycon", "a-line", "shift", "wrap", "cocktail", "sheath",
                    "sundress", "shirtdress", "romper", "trench", "parka", "peacoat",
                    "denim", "leather", "bomber", "windbreaker", "cape", "poncho",
                    "raincoat", "overcoat", "puffer", "flats", "loafers", "pumps",
                    "ankle boots", "knee-high", "combat", "stilettos", "mules", "clogs",
                    "wedges", "espadrilles", "oxfords", "slippers", "platforms",
                    "purse", "tote", "backpack", "satchel", "crossbody", "shoulder bag"
                ]
            ...
            for garment in garment_types:
                ...
                if garment_lower == item_category or garment_lower == item_subcategory:
                    ...
                    match_reasons.append(f"Same {garment} as worn by {social_proof.celebrity}")
                    break
                
                # Related category match (expanded with more categories)
                # Footwear types
                elif garment_lower in ["boots", "heels", "sneakers", "flats", "loafers", "pumps", 
                                      "stilettos", "mules", "clogs", "wedges", "espadrilles", 
                                      "oxfords", "slippers", "platforms"] and item_category == "shoes":
                    garment_match_score = 0.8
                    matched_garment = garment
                    match_reasons.append(f"Similar to the {garment} worn by {social_proof.celebrity}")
                    break
                
                # Top types
                elif garment_lower in ["blouse", "t-shirt", "tee", "tank", "camisole", "hoodie", 
                                      "turtleneck", "tunic"] and item_category == "tops":
                    garment_match_score = 0.8
                    matched_garment = garment
                    match_reasons.append(f"Similar to the {garment} worn by {social_proof.celebrity}")
                    break
                ...
                # Bottom types
                elif garment_lower in ["trousers", "culottes", "chinos", "joggers", "sweatpants"] and item_category == "pants":
                    garment_match_score = 0.8
                    matched_garment = garment
                    match_reasons.append(f"Similar to the {garment} worn by {social_proof.celebrity}")
                    break
                ...
                # Dress types
                elif garment_lower in ["gown", "maxi", "mini", "midi", "slip", "bodycon", "a-line", 
                                      "shift", "wrap", "cocktail", "sheath", "sundress", "shirtdress"] and item_category == "dresses":
                    garment_match_score = 0.8
                    matched_garment = garment
                    match_reasons.append(f"Similar to the {garment} worn by {social_proof.celebrity}")
                    break
                ...
                # Outerwear types
                elif garment_lower in ["blazer", "trench", "parka", "peacoat", "bomber", "windbreaker", 
                                      "cape", "poncho", "raincoat", "overcoat", "puffer"] and item_category == "outerwear":
                    garment_match_score = 0.8
                    matched_garment = garment
                    match_reasons.append(f"Similar to the {garment} worn by {social_proof.celebrity}")
                    break
                ...
                # Bag types
                elif garment_lower in ["clutch", "purse", "tote", "backpack", "satchel", 
                                      "crossbody", "shoulder bag"] and item_category == "bags":
                    garment_match_score = 0.8
                    matched_garment = garment
                    match_reasons.append(f"Similar to the {garment} worn by {social_proof.celebrity}")
                    break
                ...
                # Generic garment type match - partial credit for being in the same broad category
                    ...
                    (garment_lower in ["top", "shirt", "sweater", "blouse", "t-shirt", "tee"] and 
                     item_category in ["tops", "shirts", "t-shirts", "blouses", "sweaters"]) or
                    (garment_lower in ["pants", "bottoms", "jeans", "skirt", "shorts"] and 
                     item_category in ["bottoms", "pants", "jeans", "skirts", "shorts"]) or
                    (garment_lower in ["dress", "gown"] and 
                     item_category in ["dresses"]) or
                    (garment_lower in ["jacket", "coat", "blazer"] and 
                     item_category in ["outerwear", "jackets", "coats", "blazers"]) or
                    (garment_lower in ["shoes", "footwear", "boots", "heels"] and 
                     item_category in ["shoes", "footwear"])
                ):
                    garment_match_score = 0.6
                    matched_garment = garment
                    match_reasons.append(f"Related to the {garment} worn by {social_proof.celebrity}")
                    break
            ...
            if garment_match_score == 0.0 and social_proof.outfit_description:
                ...
                basic_categories = {
                    ...
                    "accessories": ["bag", "purse", "handbag", "jewelry", "scarf", "hat"]
                }
                
                for category, keywords in basic_categories.items():
                    if any(keyword in social_proof.outfit_description.lower() for keyword in keywords):
                        if category.lower() == item_category or (
                           item_subcategory and any(sub in item_subcategory for sub in keywords)):
                            score_components["garment_match"] = 0.5
                            match_reasons.append(f"Matches {category} category from {social_proof.celebrity}'s outfit")
                            break
        
        # Color matching - enhanced with partial matching
        if item.colors:  # Always try to match colors even if social_proof doesn't specify them
            ...
            if not celebrity_colors and social_proof.outfit_description:
                ...
                color_keywords = [
                    ...
                    "gray", "grey", "brown", "tan", "beige", "cream", "ivory", "navy", 
                    "teal", "burgundy", "maroon", "gold", "silver", "orange"
                ]
            ...
            if celebrity_colors:
                ...
                if color_matches:
                    ...
                    match_reasons.append(f"Matches the {color_text} color from {social_proof.celebrity}'s outfit")
                else:
                    ...
                    for group1, group2 in similar_color_pairs:
                        if (any(c in group1 for c in celebrity_colors) and any(c in group2 for c in item_colors)) or \
                           (any(c in group2 for c in celebrity_colors) and any(c in group1 for c in item_colors)):
                            score_components["color_match"] = 0.5  # Partial credit for similar colors
                            match_reasons.append(f"Similar color palette to {social_proof.celebrity}'s outfit")
                            matched_similar = True
                            break
                    
                    # Final fallback: neutral colors always get some credit
                    if not matched_similar and any(c in {"black", "white", "gray", "grey", "beige", "navy"} for c in item_colors):
                        score_components["color_match"] = 0.3
                        match_reasons.append(f"Neutral color that complements {social_proof.celebrity}'s style")
            else:
                # If no celebrity colors specified, neutral colors get some credit
                if any(c in {"black", "white", "gray", "grey", "beige", "navy"} for c in item_colors):
                    score_components["color_match"] = 0.3
                    match_reasons.append(f"Versatile neutral color that works with {social_proof.celebrity}'s style")
        ...
        if item.pattern:
            ...
            if not celebrity_patterns and social_proof.outfit_description:
                ...
                pattern_keywords = [
                    "striped", "stripes", "plaid", "checked", "checkered", "polka dot", "floral",
                    "animal print", "leopard", "zebra", "snake", "geometric", "abstract", 
                    "solid", "plain", "textured", "embroidered", "sequined", "beaded"
                ]
            ...
            if celebrity_patterns:
                ...
                if item_pattern in celebrity_patterns:
                    ...
                    match_reasons.append(f"Features the {item_pattern} pattern from {social_proof.celebrity}'s look")
                # Partial pattern match
                elif any(pattern in item_pattern or item_pattern in pattern for pattern in celebrity_patterns):
                    score_components["pattern_match"] = 0.7
                    match_reasons.append(f"Similar pattern to {social_proof.celebrity}'s outfit")
                else:
                    ...
                    for category in pattern_categories:
                        celeb_patterns_in_category = [p for p in celebrity_patterns if any(cat_pat in p for cat_pat in category)]
                        if celeb_patterns_in_category and any(cat_pat in item_pattern for cat_pat in category):
                            score_components["pattern_match"] = 0.6
                            match_reasons.append(f"Complementary pattern to {social_proof.celebrity}'s style")
                            break
            else:
                ...
                if item_pattern in ["solid", "plain"]:
                    ...
                    match_reasons.append(f"Versatile solid pattern that works with {social_proof.celebrity}'s style")
        ...
        fit_keywords = {
            ...
            "relaxed", "structured", "tailored", "unstructured", "boxy", "bodycon", 
            "a-line", "empire", "drop-waist", "peplum", "pencil"
        }
        ...
        item_fit_indicators = [tag for tag in item.style_tags if tag.lower() in fit_keywords]
        ...
        if celebrity_fit_indicators and (item_fit or item_fit_indicators):
            ...
            if item_fit and item_fit in celebrity_fit_indicators:
                ...
                match_reasons.append(f"Same {item_fit} silhouette as {social_proof.celebrity}'s outfit")
            
            # Tag-based fit match
            elif item_fit_indicators and any(fit in celebrity_fit_indicators for fit in item_fit_indicators):
                matching_fits = [fit for fit in item_fit_indicators if fit in celebrity_fit_indicators]
                score_components["silhouette_match"] = 0.8
                match_reasons.append(f"Matching {matching_fits[0]} silhouette to {social_proof.celebrity}'s style")
            ...
            # Complementary fits
                ...
                for group in complementary_fits:
                    celeb_fits_in_group = [f for f in celebrity_fit_indicators if f in group]
                    ...
                    if celeb_fits_in_group and item_fits_in_group:
                        ...
                        match_reasons.append(f"Complementary silhouette to {social_proof.celebrity}'s style")
                        break
        ...
        if social_proof.outfit_description:
            ...
            style_indicators = [
                "casual", "formal", "elegant", "chic", "minimalist", "bold", "classic",
                "vintage", "retro", "preppy", "bohemian", "boho", "edgy", "streetwear", 
                "glamorous", "sporty", "athleisure", "business", "professional", 
                "feminine", "masculine", "androgynous", "romantic", "punk", "grunge",
                "hip-hop", "sophisticated", "trendy", "timeless", "modern", "contemporary",
                "urban", "festival", "party", "lounge", "vacation", "resort", "beach",
                "office", "workwear", "cocktail", "evening", "black tie"
            ]
        ...
        if item.style_tags:
            ...
            if style_matches:
                score_components["style_match"] = len(style_matches) / max(len(style_keywords), 1)
                style_match = next(iter(style_matches))  # Get first matching style
                match_reasons.append(f"Matches the {style_match} style of {social_proof.celebrity}'s outfit")
            else:
                # Group complementary styles
                complementary_styles = [
                    {"casual", "relaxed", "comfortable", "everyday", "lounge"},
                    {"formal", "elegant", "sophisticated", "dressy", "black tie", "cocktail", "evening"},
                ...
                for group in complementary_styles:
                    ...
                    if celeb_styles_in_group and item_styles_in_group:
                        ...
                        match_reasons.append(f"Complementary {item_style} style to {social_proof.celebrity}'s {celeb_style} look")
                        break
    ...
    def generate_recommendations(
        ...
        Args:
            ...
            social_proof_context: Optional social proof context with celebrity outfit information
